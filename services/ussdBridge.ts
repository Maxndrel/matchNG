...
export const handleUSSDRequest = (phoneNumber: string, input: string): string => {
 ...
 */
export const handleSMSCommand = (command: string, userId: string): string => {
  ...
  if (cmd === 'MATCH') {
    ...
    const matches = getRecommendations(user, getJobs()).slice(0, 3);
